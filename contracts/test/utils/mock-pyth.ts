import { PriceMessageInput } from "contracts/types/VaultPricefeed"
...
export function getUpdatePriceDataCall(asset: AssetId, price: string, vaultPricefeed: VaultPricefeed, signer: Signer) {
    const priceMessage: PriceMessageInput = {
        asset,
        price: parseInt(price),
        timestamp: Math.floor(Date.now() / 1000), // Convert to seconds instead of milliseconds
    }
