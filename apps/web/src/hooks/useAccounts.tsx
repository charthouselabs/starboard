...
const useAccountsContext = () => {
  ...
  console.log('Current onboarding state:', onboardingState, 'isConnectedFuel:', isConnectedFuel, 'fuelAddress:', fuelAddress);
  
  // Auto-set onboarding state to AccountConnected when Fuel wallet connects
  useEffect(() => {
    if (isConnectedFuel && fuelAddress) {
      console.log('Fuel wallet connected, checking onboarding state:', onboardingState);
      if (onboardingState === OnboardingState.WalletConnected || onboardingState === OnboardingState.Disconnected) {
        console.log('Auto-setting onboarding state to AccountConnected for Fuel wallet');
        dispatch(setOnboardingState(OnboardingState.AccountConnected));
      } else {
        console.log('Onboarding state already set to:', onboardingState, '- not updating');
      }
      ...
      console.log('Fuel wallet disconnected, resetting onboarding state to Disconnected');
  ...
  const mockLocalDydxWallet = useMemo(() => {
    if (isConnectedFuel && fuelAddress) {
      console.log('Creating mock localDydxWallet for Fuel wallet with address:', fuelAddress);
      return {
        address: fuelAddress,
